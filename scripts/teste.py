...
def plot_positions_and_velocities_with_relative(barco_pos, barco_vel, obst_pos, obst_vel, dm, CR, title_suffix=""):
    ...
    plt.text(barco_pos[0] + 2, barco_pos[1] + 2, f"Ângulo: {angle_relative_to_line:.2f}°", color='purple')
    ...
    # Calcular produto vetorial, produto escalar e ângulo entre vetores
    cross_z = cross_product_2d(barco_vel, obst_vel)
...
for pos in obst_positions:
    for vel in obst_vel_candidates:
        # Velocidade original
        avoidance_type, cross_z, dm, CR = determine_avoidance(barco_pos, barco_vel, pos, vel)
        print(f"Sim {sim_count}: Obs pos={pos}, Obs vel={vel}, Tipo={avoidance_type}")
        ...
        avoidance_type, cross_z, dm, CR = determine_avoidance(barco_pos, barco_vel, pos, vel_oposta)
        print(f"Sim {sim_count}: Obs pos={pos}, Obs vel={vel_oposta}, Tipo={avoidance_type}")
